...
vnoremap  y:%s//gc<Left><Left><Left>\(0\)/
nnoremap  yiw:s//g<Left><Left>\(\<0\>\)/
nnoremap  yiw:%s//gc<Left><Left><Left>\(\<0\>\)/
vnoremap  di"
nnoremap  <Up>o<Down>
nnoremap <silent>  :cnzvzz:let t:qfnum = t:qfnum + 1
nnoremap <silent>  :cpzvzz:let t:qfnum = t:qfnum - 1
nnoremap   i <Right>
...
vnoremap * y/"
nnoremap * *``:%s///gn
nnoremap - :call NextColor(1)
map / <Plug>(easymotion-sn)
nnoremap <silent> / /\[^.\]
map F <Plug>(easymotion-F)
vnoremap Ff y:Grep F " ./
nnoremap Ff yiw:Grep "" \<"\> ./
nnoremap FF :Grep "" 
nnoremap N Nzz
nnoremap Q :let $MODIFIED = len(filter(getbufinfo(), 'v:val.changed == 1 && len(v:val.windows)')) | let $F = expand('%') | sh
nnoremap S :call SurroundChange()
...
nnoremap \<Down> -
nnoremap \<Up> +
vnoremap \r :<BS><BS><BS><BS><BS>execute  "resize " . (line("'>") - line("'<") + 1 + (&scrolloff * 2))zt
nnoremap \E :exe "vertical resize " . (&foldcolumn + &numberwidth + strlen(getline('.')) + 4)
nnoremap \\e :exe "vertical resize " . (&foldcolumn + &numberwidth + LongestLine('w') + 4)
nnoremap \e :exe "vertical resize " . (&foldcolumn + &numberwidth + LongestLine('') + 4)
nmap \z <Plug>(YCMHover)
nnoremap \T T
nnoremap \t t
nnoremap \F F
nnoremap \f f
nnoremap \/ /
nnoremap \cv :setlocal nowrapHv:set scbw:setlocal nowrapLjzt:set scb
nnoremap \\; W
nnoremap \; w
nnoremap \Q :only
nnoremap \q :qa!
...
nnoremap n nzz
vnoremap sd dh"_2xP
vnoremap s d:call Surround()
map t <Plug>(easymotion-t)
nnoremap z{ [{zf%
nnoremap <silent> zM :exe ":if &foldlevel | setlocal foldlevel=0 | else | setlocal foldlevel=99 | endif"
...
nnoremap <silent> <C-P> :cpzvzz:let t:qfnum = t:qfnum - 1
nnoremap <silent> <C-N> :cnzvzz:let t:qfnum = t:qfnum + 1
vnoremap <C-H> y:%s//gc<Left><Left><Left>\(0\)/
nnoremap <C-H><C-H> yiw:s//g<Left><Left>\(\<0\>\)/
nnoremap <C-H> yiw:%s//gc<Left><Left><Left>\(\<0\>\)/
nnoremap <C-G> :%s//gc<Left><Left><Left>
nnoremap <S-Tab><S-Tab> :DupTab
nnoremap <S-Tab>o :tabo
nnoremap <S-Tab><Right> :tabn
nnoremap <S-Tab><Left> :tabp
nnoremap <S-Tab><Down> :tabc
nnoremap <S-Tab><Up> :tabnew
nnoremap <S-Tab> :tabe 
...
inoremap jk 
inoremap kj 
cabbr cp cp | let t:qfnum = t:qfnum - 1
cabbr cn cn | let t:qfnum = t:qfnum + 1
cabbr ccl ccl | if exists('t:qflist') | unlet t:qflist | endif
cabbr G !git
cabbr %% %:h
cabbr VF vs | find ./**
cabbr F find ./**
...
silent only
silent tabonly
cd ~/Arduino/libraries/arduinoWebSockets-master/src
...
argglobal
%argdel
$argadd WebSocketsClient.cpp
edit WebSocketsClient.cpp
